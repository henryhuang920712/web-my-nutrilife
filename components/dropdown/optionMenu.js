    ...
    <div className="relative text-black" ref={menuRef}>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-48 bg-white border rounded-lg shadow-lg">
          ...
          <button onClick={() => router.push(`/api/auth/signout?callbackUrl=${pathname}`)} className="block w-full text-left px-4 py-2 text-red-500 hover:bg-gray-100">Logout</button>
        </div>
      )}
    </div>
  );
